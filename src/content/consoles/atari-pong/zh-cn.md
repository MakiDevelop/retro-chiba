---
lang: zh-cn
title: Atari Home Pong
qid: Q6390435
gen: 1
kind: home
maker: Atari（设计：Allan Alcorn / Nolan Bushnell / Ted Dabney）
release:
  na: "1975-12-01"
sales:
  official: "首年 15 万台（Sears 独家）/ 累计（含后续 Super Pong 变种）约 100 万台"
  community: "1975 年 $99 美元、Sears 玩具部门卖断货成为当年圣诞热销品"
  source: "Atari 与 Sears 1975-77 年销售记录"
specs:
  CPU: "**单一 pong-on-a-chip ASIC**（自研）"
  RAM: "无独立 RAM"
  分辨率: "黑白方块精灵、固定球场"
  音源: "**单一电子哔声**"
  介质: "无——游戏烙在 ASIC 内，无法更换"
  手柄: "两支内置旋钮（直接焊接于主机）"
images:
  hero:
    src: "/img/consoles/atari-pong-hero.jpg"
    alt: "Atari Home Pong 主机（单一 pong 游戏）"
    caption: "Atari Home Pong，1975 年 12 月由 Sears 独家销售，$99 美元。**单一 ASIC、单一游戏、单一声音**——但这是家用主机产业第一次站稳商业可行性的瞬间。"
    license: CC-BY-SA-4.0
    author: "Wikimedia Commons contributor"
    source_url: "https://commons.wikimedia.org/wiki/File:Atari_Home_Pong_(1975)_1.jpg"
hardware_variants:
  - name: "Sears Tele-Games Pong"
    release: "1975"
    type: "Sears 独家首发版"
    note: "Home Pong 最早不是以 Atari 品牌大规模进入美国家庭，而是挂着 Sears Tele-Games 的百货通路品牌。1975 年的家用电子游戏还不是电子娱乐产业，而是圣诞玩具货架上的新奇商品。"
  - name: "Atari Home Pong C-100"
    release: "1975"
    type: "Atari 自有品牌版"
    note: "C-100 把街机 Pong 压缩成一颗专用芯片与两个旋钮，没有 CPU、没有卡带，也没有可更新软件。它的成功证明「一台只能玩一个游戏的盒子」也能成为家庭消费品。"
  - name: "Super Pong"
    release: "1976"
    type: "多模式 Pong 变种"
    note: "Super Pong 加入更多球场与玩法选项，代表单游戏主机时代的典型升级方式：不是换游戏，而是在同一套硬件逻辑里多塞几种 Pong 规则。"
  - name: "Ultra Pong / Pong Doubles"
    release: "1977"
    type: "四人与多变体机型"
    note: "Ultra Pong 与 Pong Doubles 把旋钮、模式与多人游玩推到极限。它们是专用 Pong 机的最后繁荣，也正好撞上 Atari 2600 代表的可换卡带时代。"
  - name: "Pong-on-a-chip 授权与仿制机"
    release: "1976-1978"
    type: "专用芯片生态"
    note: "General Instrument 等厂商推出低成本 Pong-on-a-chip 后，世界各地出现大量仿制与授权变体。Pong 从 Atari 商品变成一整个白牌家用机市场，也让单游戏主机迅速商品化。"
notable_games:
  - "Pong（主机唯一游戏）"
  - "Super Pong / Ultra Pong / Pong Doubles（变种主机系列）"
---

Atari Home Pong 的故事不是从 1975 年开始的，是从 1972 年那场 Magnavox 经销商 demo 开始的。**Nolan Bushnell**（Atari 创办人）在 1972 年 5 月看完 Magnavox Odyssey 的 Table Tennis 后，回到 Atari 命令工程师 **Allan Alcorn** 做一个「**简化练习**」——把这个概念做成街机版。Alcorn 三个月内完成原型，安装在加州 Sunnyvale 的 Andy Capp's 酒吧。**第一晚机台就坏了——不是电路问题，是投币口被塞满 25 美分硬币**。1972 年街机版 Pong 是 Atari 的第一笔大钱，三年内卖出 19,000 台街机。

家用版 Home Pong 是 1974 年 Alcorn 带领的家用化项目——把整个 Pong 街机压缩进**一颗自研 ASIC 芯片**。没有 CPU、没有 RAM、没有可编程逻辑，**整个游戏就是这颗 ASIC 内置的硬件电路**。1975 年圣诞季由 **Sears Roebuck**（美国最大百货）独家销售，售价 $99。**Sears 玩具部门的订单是 Atari 史上第一笔 7 位数的订单——15 万台首年销售在 Sears 卖断货**，成为 1975 年圣诞节最热销玩具。

但「Home Pong 是抄 Magnavox Odyssey 的」这件事，在 1974 年就被 Magnavox 法务注意到。1974 年 Magnavox 起诉 Atari 专利侵权，**1976 年双方达成历史性和解**——Atari 一次付 70 万美元换取 Pong 相关专利的永久授权。**这场官司的判例至今仍影响主机产业所有专利战**。Atari 在 1976-1977 推出 Super Pong、Ultra Pong、Pong Doubles 等十多款变种，每款都是**单一 ASIC 内置一款 Pong 变体游戏**——这是「主机 = 一款游戏」时代的最后疯狂。

1977 年 Atari 推出真正能换卡带的 Atari 2600，**Pong 系列瞬间过时**。但 Home Pong 在 1975 年完成的事比销量更重要——它是**家用主机产业第一次站稳商业可行性的瞬间**。Magnavox Odyssey 证明「家用机可行」，Atari Home Pong 证明「**家用机可赚钱**」。没有 Home Pong 的成功，就没有 Atari 投入 Atari 2600 开发的勇气，也没有后来 1980 年代的整个产业。
