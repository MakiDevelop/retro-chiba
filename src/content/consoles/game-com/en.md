---
lang: en
title: Tiger Game.com
qid: Q2498396
gen: h
kind: handheld
maker: Tiger Electronics
release:
  na: "1997-08-21"
sales:
  official: "~300,000 worldwide (1997–2000)"
  community: "**The first home or handheld console with a stylus + dial-up internet** — seven years ahead of the Nintendo DS"
  source: "Tiger 2000 exit cumulative"
specs:
  CPU: "Sharp SM8521 @ 10 MHz (8-bit)"
  Display: "**200×160 monochrome LCD with a touch overlay** — touchscreen in a console first"
  RAM: "8 KB system + 64 KB VRAM"
  Audio: "4-channel PSG"
  Media: "ROM cartridge + **modem cartridge** (dial-up networking)"
  Battery: "Four AAs for ~10 hours"
  Network: "**Built-in 14.4K modem cartridge** — could check email"
images:
  hero:
    src: "/img/consoles/game-com-hero.jpg"
    alt: "Tiger Game.com black landscape handheld with touch overlay and stylus"
    caption: "Tiger Game.com, North American launch August 1997 at $69. **The first console with a stylus and a dial-up modem** — seven years ahead of the DS on touch, eight years ahead of the PSP on networking. The catalog let it down: Tiger Electronics was a toy company (Furby, talking-LCD pets) without game-industry experience."
    license: PD
    author: "Evan-Amos"
    source_url: "https://commons.wikimedia.org/wiki/File:Tiger-Game-Com-FL.jpg"
  gallery:
    - src: "/img/consoles/game-com-tiger-game.com-with-cartridges.jpg"
      author: "Matthew Paul Argall"
      source_url: "https://commons.wikimedia.org/wiki/File:Tiger%20Game.Com%20with%20cartridges.jpg"
      alt: "Tiger Game.com system with cartridges"
      caption: "Tiger Game.com with its cartridge accessories — roughly 20 titles shipped over the platform's life, including the notoriously catastrophic console-grade IP downports Resident Evil 2, Sonic Jam, and Mortal Kombat Trilogy."
      license: PD
hardware_variants:
  - name: "Tiger Game.com standard (71-541)"
    release: "1997 NA"
    type: "Original touchscreen handheld"
    note: "Released in North America in August 1997 at $69 — **the world's first touchscreen handheld** (seven years ahead of the Nintendo DS), a 3.5-inch monochrome LCD without backlight, equipped with a thin stick stylus. Tiger Electronics had spent the 1980s and early 1990s as a maker of LCD single-game handhelds (Tiger handheld Sonic, Tiger handheld Street Fighter II were 1980s American childhood memories), and this was its attempt to step up to a real cartridge-based platform. But the screen update rate of only 5-15 fps, the monochrome display, and the poor touch response all hit the failure zone simultaneously."
  - name: "Tiger Game.com Pocket Pro (71-558)"
    release: "1999 NA"
    type: "Shrunken revision"
    note: "Released in 1999 at $30, the **Pocket Pro** shrank the chassis substantially, added a backlight (fixing the original's dim, hard-to-read screen), and removed the stylus garage (the stylus became external) — but it also dropped one of the original's two cartridge slots (now just one). **Cost reduction at the expense of features** — Tiger was trying to use price to rescue a market already crushed by the Game Boy Color (1998), but Pocket Pro just confused customers further: \"**why buy a console where they removed features?**\" Lifetime sales are estimated under 300,000 units."
  - name: "Console-grade IP port disasters (Resident Evil 2 / Sonic Jam / Mortal Kombat Trilogy)"
    release: "1997-2000"
    type: "Catastrophic software lineup"
    note: "Game.com shipped roughly 20 cartridges over its life, but **its claim to fame is the \"console-grade IP port disaster\" trifecta** — Resident Evil 2 (1998, the monochrome 5fps degradation of the PS1 original, possibly the most absurd cross-platform port to share a name), Sonic Jam (1998, ported from Saturn), and Mortal Kombat Trilogy (1998, ported from PS1 / N64). **These ports proved Game.com developers should never have attempted console-grade IP**, but Tiger marketing insisted: \"Game Boy doesn't have RE2, Game.com does\" — the result is the most famous example of console IP port failure in history."
  - name: "Touchscreen concept lineage to DS (2004)"
    release: "1997 → 2004"
    type: "Conceptual lineage"
    note: "Game.com's most important historical legacy is not commercial performance but **the early implementation of the touchscreen handheld concept** — seven years ahead of Nintendo DS (2004), ten years ahead of iPhone (2007). Tiger Electronics was acquired by Hasbro in 1998, ending its handheld business, but **the \"touchscreen handheld\" concept** was redefined by Nintendo DS in 2004 with the same dual-screen 1.5-inch + 5-inch architecture (**DS = Dual Screen**). Game.com is the failed pioneer; the DS is the successful heir — the cleanest \"pioneer's tragedy\" lineage in commercial tech history."
  - name: "Game.com accessories: dial-up Modem / Internet Cartridge"
    release: "1997-1998"
    type: "Online and accessories"
    note: "Game.com had two little-known accessories — the **Game.com Modem** ($80, a 14.4k dial-up modem for going online and reading email) and the **Internet Cartridge** (a basic browser cartridge). **An exceptionally early attempt at \"handheld online\"** for any home console or handheld vendor — four years ahead of Mobile WonderGate (2001), contemporaneous with Nintendo Game Boy Camera link features (1998). But Game.com's commercial collapse was so fast that lifetime accessory sales are estimated at under 5,000 units worldwide. Today they're high-value rarities in Tiger Electronics collecting."
notable_games:
  - "Lights Out (Tiger, 1997 — pack-in)"
  - "Resident Evil 2 (Capcom, 1998 — **the platform's notorious port disaster**)"
  - "Sonic Jam (Sega, 1998 port)"
  - "Mortal Kombat Trilogy (Tiger, 1998 port)"
  - "Solitaire (Tiger, 1997 — pack-in)"
---

On August 21, 1997, Tiger Electronics launched the Game.com in North America at $69 (later cleared at $29). **It was the first home or handheld console to ship with a stylus, a touch overlay, and a dial-up modem cartridge** — **seven years ahead of the Nintendo DS on touchscreen input and eight years ahead of the PSP on built-in networking**. At the level of pure concept, the Game.com correctly anticipated every major direction handheld design would take over the subsequent twenty-five years, **but it was delivered by the wrong company, with the wrong technology, at the wrong moment, and accompanied by the wrong software** — the most complete textbook example of "right ideas given to entirely wrong execution" in handheld history.

Tiger Electronics was a well-known **North American electronic-toy maker** of the 1980s and 1990s. The company's product line included: **single-game LCD handhelds** (the Tiger Handheld series — small dedicated arcade-style toys), **Furby** (the 1998 cultural-phenomenon talking plush toy), the **Talking series** (Talking Yoda, Talking Vader, Talking Sega), and **HitClips** music toys. **Tiger had no experience whatsoever in dedicated game-console design**: they ran the Game.com through the same supply chain that produced Furbys, and from product design through software pipelines through retail positioning they were comprehensively outside the game industry's professional norms.

In hardware terms, the Game.com sat awkwardly between eras. A **Sharp SM8521 at 10 MHz** (an 8-bit core that was nominally stronger than the contemporary Game Boy but weaker than the seven-year-old Game Gear), a **200×160 monochrome LCD with a resistive touch overlay** (the first touchscreen in any console), 4-channel PSG audio, ROM cartridges, and a **14.4 K modem cartridge** that allowed the system to dial Tiger's own servers and access a small set of email and curated web pages. **In 1997, this combination was a six-year-late Game Gear-class system retrofitted with touch and a modem** — both concepts correct in principle, neither well executed: the monochrome LCD's slow refresh produced ghosting on touch input, and the modem could only reach Tiger's own limited servers.

**The actual catastrophe was the software catalog**. Tiger had no game-industry development culture, no proper SDK, and no third-party trust. **The most famous disaster was the *Resident Evil 2* port** (Capcom-licensed, Tiger-developed, 1998) — Capcom's console-grade survival horror IP rendered in 200×160 monochrome **with crude sprite work, no animation, no surviving audio fidelity**, zombies reduced to amorphous black blobs and Leon and Claire reduced to abstract rectangles. **For twenty-five years it has been the canonical internet meme for "console-grade IP catastrophically downported to a handheld"** — Angry Video Game Nerd (James Rolfe) covered it in 2008, and it has been a recurring touchstone in retro-gaming discourse ever since. Other disasters in the same vein included the ***Sonic Jam*** port (Sega, 1998 — Saturn-era content forced through a monochrome handheld) and ***Mortal Kombat Trilogy*** (Tiger, 1998 — a PS1-era 3D fighter rendered as static digitized stills).

Commercially, the Game.com reached **roughly 300,000 units worldwide** (1997–2000) — the lowest sales figure of any handheld released in the 1997–1999 window (Game Boy Color: ~50M, WonderSwan: ~3.5M, NGPC: ~2M, Game.com: ~0.3M). **Tiger discontinued the platform in 2000**: Hasbro had acquired Tiger Electronics in 1998 (primarily for the Furby and Talking-toy product lines), and within Hasbro the Game.com was treated as a failed acquisition asset and wound down quickly.

**But the Game.com's conceptual legacy was vindicated comprehensively over the next twenty-five years** — touch input (DS 2004 / iPhone 2007 / iPad 2010 / Switch 2017), built-in networking (PSP 2004 / DS Wi-Fi Connection 2005 / smartphones throughout), and remote/cloud delivery (Stadia / GeForce Now 2019 onward). **The Tiger Game.com is the most extreme case in handheld history of "right concept assigned to entirely the wrong company"**: every direction that subsequent dedicated handheld design would take, Tiger had imagined in 1997 — but Tiger was a toy company, not a game company, and so those correct directions in their hands produced the *Resident Evil 2* meme. **That irony is precisely what gives the Game.com its instructive position in retro-gaming history**.
